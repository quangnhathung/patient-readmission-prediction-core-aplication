...
def encode_admission_type(val) -> str:
    ...
    mapping = {
        ...
        5: "Not_Available", 6: "NULL", 7: "Trauma", 8: "Not_Mapped",
    }
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "admission_source_id" in df_fe.columns:
        ...
        df_fe = df_fe.drop(columns=["admission_source_id"])
    ...
    if all(c in df_fe.columns for c in ["number_outpatient", "number_emergency", "number_inpatient"]):
    ...
    if all(c in df_fe.columns for c in ["time_in_hospital", "num_medications"]):
        df_fe["treatment_complexity"] = df_fe["time_in_hospital"] * np.log1p(df_fe["num_medications"])

    if all(c in df_fe.columns for c in ["num_lab_procedures", "time_in_hospital"]):
        df_fe["lab_to_days_ratio"] = df_fe["num_lab_procedures"] / (df_fe["time_in_hospital"] + 1)
